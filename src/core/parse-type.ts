...
export function parseTypeAlias(fileName: string, fileSource: string, config: IDL2ConfigType) {
  ...
  const traverseNode = (node: ts.Node) => {
    ...
    if (ts.isTypeAliasDeclaration(node)) {
      ...
      const handleTypeLiteral = (node: ts.Node) => {
      ...
      ts.forEachChild(node, handleTypeLiteral);
    }
